...
export basedir="/scratch/lmx/ThaTract/"
export codedir="/dipc/lmx/GIT/ThaTract"
export tool=$1
export sub=$2
export ses=$3
export analysis=$4
...
printf "\n\n It will use basedir:$basedir and tool:$tool \n\n"
	...
	if [ "$noqsub" = "run" ]; then
		...
		module load Singularity/3.5.3-GCC-8.3.0
	...
	if [${noqsub} = ""]; then
		printf "#########################################"
            ...
            -o /dipc/lmx/logs/t-${tool}_a-${analysis}_s-${sub}_s-${ses}.o \
            -e /dipc/lmx/logs/t-${tool}_a-${analysis}_s-${sub}_s-${ses}.e \
			-v tool=$tool,path2subderivatives=$path2subderivatives,path2config=$path2config \
